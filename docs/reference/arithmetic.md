...
This document is normative and extends chapter 6 of [the specification](specification.md).
...
`calc(...)` in one of these logic positions MUST be quoted when it is intended
as literal text. `versions 1..2` selects model versions, not a language release.
...
public-data dependency checks. Hiding a field read inside `calc` does not
...
Use the conformance suite to verify implementation behavior.
