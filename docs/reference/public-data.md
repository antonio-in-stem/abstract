# Public scalar export

`@public` lets an author nominate a declared scalar for an opt-in export.
Ordinary compilation still emits the same data, including non-public fields,
and applies the same validation and logic. A nomination is neither a privacy
boundary nor permission for a consumer to override a value automatically.
...
editor provenance. See [the analysis protocol](editor-protocol.md#optional-public-inventory-1).
...
does not run YAML overrides, provide Java projections, sandbox downstream code,
or prevent observations or copying of the compiled product. Consumers may use
the optional Java reader to load compiled JSON, but that reader does not apply
public values or create an override policy.
