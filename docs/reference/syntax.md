...
[arithmetic](arithmetic.md). Use `calc(...)` in a derive value or condition
operand; ordinary instance values remain data.

A lookup table for people editing `.ab` and `.abt` files. Every entry names the
section of [the specification](specification.md) that defines it; the specification is normative
and this page is not. The grammar rule names in the right-hand columns are
defined in [the grammar](grammar.ebnf).

For a narrative introduction read [the language guide](../language.md)
instead. Runnable versions of the examples below live in
[`examples/features/`](../../examples/features/README.md).
...
See [compiled data](compiled-data.md) for the consumer contract, and SPEC 8 for the
byte-exact rules.
