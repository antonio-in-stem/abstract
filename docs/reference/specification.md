...
Status: normative. This document defines the Abstract data language. Arithmetic
expressions in logic use `calc(...)`; their numeric contract, work charges, and
diagnostics are defined in [arithmetic.md](arithmetic.md). The optional
`@public` scalar export is specified in [public-data.md](public-data.md).
...
**P1; Minimal.** The language has one way to express each thing. Features that can be expressed by composing existing features are not added. Abstract has no imports, no free-key maps, no user-defined functions, no nested lists, no per-schema version axes and no comments in output.
...
```abstract
schema Option { id_of_pack: ref(Pack) }
```
```abstract
Option :: @id.dragon
...
```
```json
{ "template": "Option", "id": "dragon", "id_of_pack": "winter_2026" }
```
...
- An `@optional` list field that is absent is **omitted** from the output, exactly like any other absent optional field. It does not become `[]`.
...
There is no lenient mode in Abstract. `--allow-unknown` does not exist.
...
There is no "direct form" (no `abstract file.ab Template.abt JSON`): a command word is always required.
...
| E407 | Clone across templates | `Instance '{id}' is a '{other}'; a clone source must use the same template '{template}'.` | cloning a `Pack` from an `Option` |
...
| E432 | `ref` target has the wrong schema | `Reference '{id}' at {context}.{field} is a '{actual}', expected a '{expected}'.` | pointing at an `Option` |
...
The runnable [product-options example](../../examples/features/product-options/)
shows a collection and its options across model `versions`. It includes source
files and canonical JSON, YAML, and RAW output. Sections 7 and 8 define the
normative compilation and output rules.
...
5. The nonce construction MUST be documented completely. The compiler uses 96 bits from the operating system's cryptographically secure random source, fails if that source is unavailable, and uses no clock or process-local counter. Random selection has a collision probability; it is not a uniqueness proof. See [bundle security](../../java/SECURITY.md) for the per-key usage limit.

Creating a sealed bundle requires a 32-byte key
...
creation is refused. This optional tooling rule does not change the ABX1 format.
Legacy passphrase derivation remains available for decoding.
...
14. The syntax grammar MUST cover the language: nested braces inside `schema`, `logic` and instance body blocks, every type keyword of §4.4, `versions`, `@since`, `@removed`, `ref` and `$(Schema)`.
