...
The normative text is [`SPEC.md`](SPEC.md) sections 2.7, 7.5 and 8. Where this
page and the specification disagree, the specification wins.

Runnable projects for every example here are in
[`examples/`](examples/README.md): [`formats`](examples/formats) for the three
renderings, [`versions-overlays`](examples/versions-overlays) and
[`instance-windows`](examples/instance-windows) for the overlay rules.
...
SPEC 7.5. A project carries every version of its data in one document.
...
[`examples/overlays-multi-match`](examples/overlays-multi-match) exercises both:
...
Project [`examples/instance-windows`](examples/instance-windows), with the same
schema:
