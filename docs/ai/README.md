...
`site/` mirrors `docs/`. When a documented rule changes, update the matching
page under `site/` in the same change: the topbar navigation, the docs hub, the
grammar reference and the diagnostics reference all restate rules that live in
SPEC.

- The design is fixed: reuse the existing classes in `site/assets/site.css`
  (`topbar`, `nav`, `doc-hero`, `doc-shell`, `sidebar`, `doc-content`, `cards`,
  `card`, `table`, `callout`, `terminal`, `split`, `section`, `eyebrow`,
  `lede`, `mini-list`, `page-links`, `footer`). Do not invent new classes or
  add stylesheets.
- Every page loads `assets/site.css` and `assets/site.js` with the correct
  relative depth, and marks its own nav entry with `aria-current="page"`.
- `site/downloads/` holds a prebuilt binary. **It is not regenerated by a docs
  change** — leave the directory untouched and let the release process replace
  it.
