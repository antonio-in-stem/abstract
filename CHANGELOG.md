...
- Added explicit `calc(...)` arithmetic in logic: checked integer operations,
  finite floating-point calculations, numeric functions and list aggregation.
  Evaluation shares a bounded work budget with logic. See `docs/ARITHMETIC.md`.
- Arithmetic participates in compiler-owned references, renaming, evaluated
  value hovers and public-field dependency checks.
- Added six progressive exercises with separate starters and explained solutions.
- VS Code extension 1.7.2 packages contextual arithmetic help, corrected
  workspace setup instructions, the new Abstract identity and a product guide.
- Added executable CUE comparison, disk-resource regression coverage and release CI.

Compatibility: language 1.2 treats glued lowercase `calc(` in a logic expression
as arithmetic. Previously literal text with that spelling in those positions
must be quoted. Ordinary instance text is unchanged.
